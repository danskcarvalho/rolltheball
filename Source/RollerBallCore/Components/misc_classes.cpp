...
node_without_transform::node_without_transform(class node* n){
    _node = n;
}
...
class node* node_without_transform::node() {
    return _node;
}
...
void multiple_nodes::describe_type() {
    ...
    if(_original)
        ...
        _all_nodes = std::all_of(_objects.begin(), _objects.end(), [=](const typed_object* obj){ return dynamic_cast<const node*>(obj); });
